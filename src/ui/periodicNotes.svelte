<script lang="ts">
	...
	import { periodicFocusRequest, periodicFocusBackRequest, recentFilterFocusRequest } from "src/store";
    ...
    // Whether the recent files section is rendered below (Tab forwards into it)
    export let recentEnabled: boolean = false
    ...
    function handleListKeydown(e: KeyboardEvent): void {
        if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
            ...
            if (e.shiftKey) {
                ...
                // Tab (forward loop): into the recent files filter
                recentFilterFocusRequest.update(n => n + 1)
            } else {
                HomeTabSearchBar?.focusSearchbar()
            }
